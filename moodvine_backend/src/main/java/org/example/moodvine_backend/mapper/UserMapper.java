...
public interface UserMapper extends BaseMapper<User> {
    ...
    @Update("update user set nick_name=#{newNickname} where user_id=#{userId}")  //更新user中指定uid的用户的昵称
    void updateNickname(String userId,String newNickname);

    @Update("update user set signature=#{newSignature} where user_id=#{userId}")  //更新user中指定uid的用户的签名
    void updateSignature(String userId,String newSignature);

    @Update("update user set avatar=#{avatar} where user_id=#{userId}")  //更新user中指定uid的用户的头像
    void updateAvatar(String userId, String avatar);

    @Update("truncate table feedback")  //清除feedback数据库内容
    void clear();

    @Update("update user set nick_name=#{nickName}, password=#{password}, avatar=#{avatar}, signature=#{signature}, gender=#{gender}, photo=#{photo} where id=#{id}")
